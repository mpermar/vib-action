...
import fs from 'fs';
import {createPipeline, getExecutionGraph, getToken, loadConfig, reset, runAction} from '../src/main'
...
describe('VIB', () => {
  ...
  it('No CSP_API_TOKEN throws an error', async () => {
    ...
    try {
      ...
      expect(getToken).rejects.toThrow(new Error('CSP_API_TOKEN secret not found.'));
    } finally {
  ...
  it('No CSP_API_URL throws an error', async () => {
    ...
    try {
      ...
      expect(getToken).rejects.toThrow(new Error('CSP_API_URL environment variable not found.'));
    } finally {
...
//      Using defaults is more resilient and friendlier than forcing users to define env vars.
  it('No VIB_PUBLIC_URL throws an error', async () => {
    ...
    try {
      ...
      expect(createPipeline).rejects.toThrow(new Error('VIB_PUBLIC_URL environment variable not found.'));
    } finally {
